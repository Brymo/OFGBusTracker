...
async function getBusData() {
  ...
  const trimmedBusData = busData.stopEvents
    .filter((stopEvent, index) => stopEvent.isRealtimeControlled && index < numResults)
    .map((stopEvent) => {
      const {
      ...
      return {
        ...
        departureTimeEstimated: departureTimeEstimated && isoToSydney(departureTimeEstimated),
